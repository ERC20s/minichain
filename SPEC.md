...
- Acceptance rule (src/node.ts, gossip "blk" handler): after the header signature, the proposer election and the recomputed-merkleRoot check (see "Validation order in block acceptance" below), the node runs verifyTransaction over every transaction in the block and RETURNS on the first failure. The tip does not move and nothing is re-broadcast, so an unauthorised transfer does not propagate through this node. An empty transaction list passes, as it always did.
- Ordering: the per-transaction check sits after the root check on purpose — the root check is cheap and rejects a mismatched list before any transaction signature work is done — and both sit after the header signature and proposer checks, so an unauthenticated block never buys per-transaction work at all.
...
- Acceptance rule (src/node.ts, gossip "blk" handler): `this.balances.stage(txs)` runs immediately AFTER the nonce staging and immediately BEFORE the accept point; the handler returns on null. `commit` is called only at the accept point, beside `this.tip = blk` and `this.nonces.commit(staged)`, so tip, nonces and balances can never disagree and a block dropped by a later check cannot spend balances the chain never spent.
...
- Placement: the three checks run immediately after the height and parentHash linkage checks and BEFORE the header signature, the proposer check, the Merkle recompute, the transaction signatures and nonce and balance staging — cheapest first, and no state is touched. A failing block is dropped exactly as any other failing block: the tip does not move and nothing is re-broadcast.
...
Validation order in block acceptance (new)
- Principle: CHEAP AND AUTHENTICATING FIRST. The gossip "blk" handler in src/node.ts validates a block in strictly increasing cost, and every authenticating check that costs constant work runs before any check whose cost grows with the size of the block.
- Threat: src/gossip/ws.ts accepts any inbound WebSocket connection and authenticates nobody; a frame is only bounded in size (64 KiB of payload). A node's tip hash is public, so any peer could send a block with correct height and parentHash, a legal timestamp and a correctly computed Merkle root over a few hundred junk-signed transactions. Under the old order that frame cost the node the whole Merkle recompute plus one ed25519 verification PER TRANSACTION, plus nonce and balance staging, before it reached the check that the block carried no valid proposer signature at all. One cheap frame bought N expensive verifications, repeated as fast as the socket allowed.
- Order, as enforced now:
  1. linkage: height === tip.height + 1, parentHash === blockHash(tip).
  2. timestamp bounds: safe non-negative integer, not before the tip, not beyond now() + maxFutureDriftMs.
  3. header authentication: the envelope must carry sig and pubKey, and the ed25519 signature must verify over canonicalBlockEncoding of the header with proposerPublicKey = the envelope key.
  4. proposer election: when the node holds a non-empty validator set, selectValidator(validators, proposerSeed(tip)) must equal publicKeyToHex(pubKey); an election that returns null fails closed.
  5. transaction commitment: transactionLeaves(txs) (a CanonicalEncodingError is a drop) and merkleRoot(txBytes) === blk.merkleRoot.
  6. per-transaction authorisation: verifyTransaction over every transaction.
  7. nonce staging, then balance staging.
  8. accept: the tip moves, nonces and balances commit, the block is re-broadcast.
- Steps 1 to 4 are constant work; 5 to 7 are proportional to the transaction count. Rejecting a forged or unelected block therefore costs ONE signature verification instead of N + 1.
- Semantics unchanged: acceptance is the conjunction of exactly the same predicates, every check is free of side effects until step 8, and no check reads state another check writes. The set of accepted blocks is identical; only the cost of rejecting one changes. In particular the proposer seed is still proposerSeed(this.tip) read while this.tip is still the parent — this.tip moves only at step 8.
- Not a wire-format change and not a rule change: no encoding, signature, leaf, root or block hash changes, and no block that was accepted before is rejected now (or the reverse).
- What this does NOT fix: there is still no peer authentication, no per-peer rate limiting and no cap on how many frames a socket may send. A flood of cheaply-rejected frames is still a flood; this change only removes the amplification factor. Rate limiting is separate work.

Files added or changed for validation order
- src/node.ts (the m.sig/m.pubKey presence check, the canonicalBlockEncoding header verification and the validator-set/selectValidator election moved to run immediately after the timestamp bounds and before transactionLeaves; comments record the order and why)
- test/node-validation-order.test.ts (new: src/tx is mocked with its real implementation and verifyTransaction wrapped in a jest.fn, and a bare WebSocket delivers one block frame to the node under test — an unsigned block, a forged header signature and a correctly signed block from an unelected signer each cost ZERO per-transaction verifications and leave the tip at genesis, while a valid eight-transaction block is verified eight times and moves the tip, and a tampered transaction inside a properly signed block is still rejected)
- SPEC.md (this section; the proposer-enforcement, transaction-authorisation, nonce and balance sections now point at it for the order)
