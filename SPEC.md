...
- Not covered yet: nonce replay (the same signed transaction can appear in two blocks), balances and double spends, fees, and any state at all. The signature proves consent to THIS transfer, not that it is affordable or fresh. Those are separate cycles.
